...
df1 <- filter(df_summarise, vacc_per_week == 0.05, t_d3 == 180, vfr == 3.9) %>%
  filter(strategy_name %in% c("10y+ 2 doses, no booster", "10y+ 2 doses, booster 60y+", "10y+ 2 doses, booster 10y+")) %>%
  mutate(strategy_ind = if_else(strategy_name == "10y+ 2 doses, no booster", 1, ifelse(strategy_name == "10y+ 2 doses, booster 60y+", 2, if_else(strategy_name == "10y+ 2 doses, booster 10y+", 3, 0)))) %>%
  mutate(Rt_lift_t = factor(Rt_lift_t, levels = c("Sept '21 lift", "Nov '21 lift", "April '22 lift"))) %>%
  filter(Rt_lift_t == "Sept '21 lift" | (Rt_lift_t == "Nov '21 lift" & (strategy_name %in% c("10y+ 2 doses, booster 60y+", "10y+ 2 doses, booster 10y+"))) | (Rt_lift_t == "April '22 lift" & strategy_name == "10y+ 2 doses, booster 10y+")) %>%
  mutate(strategy_name = factor(strategy_name, levels = c("10y+ 2 doses, no booster", "10y+ 2 doses, booster 60y+", "10y+ 2 doses, booster 10y+"), ordered = TRUE))
...
ggplot(data = filter(df_summarise, max_Rt_omicron == 3,
                     strategy_name != "Pre-vaccine introduction"), aes(x = as.Date(date), y = vaccines_t / target_pop)) +
  geom_line() +
...
df_doses <- df1 %>%
  filter(vaccine_doses != "Pre-vaccine introduction", max_Rt_omicron == 3, t_d3 == 180) %>%
...
p3 <- ggplot(data = df1_doses_month, aes(x = as.Date(date), y = value/target_pop*100/strategy_ind, fill = dose)) +
  ...
  labs(x = "Time", y = "Vaccinated (%)", fill = "Dose", col = "Rt lift", linetype = "Rt lift")
p3
ggsave("plots/rq3_hic_doses_lift.png", p3, width = 8, height = 3)
