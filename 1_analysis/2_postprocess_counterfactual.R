...
df_deaths <- df %>%
  ...
  group_by(repetition, income_group, target_pop, max_coverage, age_groups_covered, age_groups_covered_d3, vaccine_doses, vacc_start,  dose_3_fold_increase, vacc_per_week, t_d3, max_Rt) %>%
  summarise(deaths = sum(D_count),
            .groups = 'drop') %>%
  group_by(income_group, target_pop, max_coverage, age_groups_covered, age_groups_covered_d3, vaccine_doses, vacc_start, dose_3_fold_increase, vacc_per_week, t_d3, max_Rt) %>%
...
df <- df %>%
  group_by(income_group, target_pop, max_coverage, age_groups_covered, age_groups_covered_d3, vaccine_doses, vacc_start,  vacc_per_week, t_d3, max_Rt) %>%
...
df_summarise <- df %>%
  unnest(cols) %>%
  select(-c(deaths, prop_R, inc)) %>%
  group_by(timestep, income_group, target_pop, max_coverage, age_groups_covered, age_groups_covered_d3, vaccine_doses, vacc_start, deaths_med, deaths_lower, deaths_upper, inc_med, inc_lower, inc_upper, prop_R_med, total_doses_med, vacc_per_week, t_d3, max_Rt) %>%
