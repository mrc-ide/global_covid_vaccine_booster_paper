...
run_scenario <- 
  function(fit,
           ...
           max_Rt_omicron,
           R0_t3_in,
           hosp_scal_omicron = 0.65,
           ICU_scal_omicron = 0.5){
    ...
    if (R0_t5 < R0_t3) {
      rt <-    c(1.1,   1.1,   0.2,   0.2,  max((max_Rt_omicron - max_Rt),0.2), max((max_Rt_omicron - max_Rt),0.2), max_Rt_omicron)
    } else {
    ...
    #for now assume that the decay rate for natural infection is the same as for D1 - we will have data on this shortly from HK
    dr_vec_inf <- dr_vec %>%
      select(-t,-D1,-D2,-D3) 
