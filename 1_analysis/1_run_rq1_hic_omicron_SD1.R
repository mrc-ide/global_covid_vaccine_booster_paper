...
t_d3 <- t_d3_set
max_Rt <- 3.5
#max_Rt_omicron <- 4.25 #c(4.25,4.5,75)
...
corr_params <- data.frame(vfr = vfr[order(vfr)], max_Rt_omicron = c(3.25, 3.5, 3.75), hosp_scal_omicron = c(0.3,0.5,0.6))
...
scenarios <- expand_grid(fit = fit,
  ...
  filter(!(vfr != central_vfr & t_d3 != 180)) %>%
 # filter(!(vfr != central_vfr & hosp_scal_omicron != 0.5)) %>%
 # filter(!(t_d3 != 180 & hosp_scal_omicron != 0.5)) %>%
  unique()
