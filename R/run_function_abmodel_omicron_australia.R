...
run_scenario <- 
  function(fit,
    ...
    # these time points are used further down to define the start of the simulation and the small pulses that reduce stochasticity
     ...
     R0_t7b <- as.Date(x = "10/1/2021", format = "%m/%d/%Y")
    ...
    R_profile <- readRDS("data/R_profile_australia.RDS")
    ...
    new_dates <- R_profile$date
    new_rt <- R_profile$Rt
    
    rt_out <- safir::interpolate_rt(dates = new_dates, rt = new_rt, max_date = tmax_date)
    ...
    #  first pulse, spread out hazard of 0.01 over 20 days right before opening up
    t_spread <- 10
    lambda_tt <- as.integer(difftime(as.Date(x = vfr_time1, format = "%m/%d/%Y"), R0_t0 - 1))
    ...
    # Save output
 #   saveRDS(df, paste0("raw_outputs/output_", name, "/test_df.rds"))
